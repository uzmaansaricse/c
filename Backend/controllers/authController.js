...
const addBook = async (req, res) => {
    try {
        ...
        if (!["4-5 Days", "5-7 Days", "7-10 Days"].includes(shippingDetails)) {
            return res.status(400).json({ message: "Shipping Details must be '4-5 Days', '5-7 Days' or '7-10 Days'" });
        }

        if (!title || !author || !price || !offerPrice || !description || !isbn || !shippingDetails || !pageNumber || !language || !weight) {
...
const getBooks = async (req, res) => {
    try {
        ...
        const formattedBooks = books.map((book) => ({
            ...
            isbn: book.isbn,
            shippingDetails: book.shippingDetails,
...
const saveorder = async (req, res) => {
    try {
        ...
        const newOrder = new Order({
            userId: new mongoose.Types.ObjectId(), // JWT se set karoge
            books: cart.map(book => ({
